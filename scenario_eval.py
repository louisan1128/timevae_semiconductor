...
def evaluate_model(
    ...
    with torch.no_grad():
        for i in range(len(X)):
            ...
            macro_x = torch.tensor(MACRO_X[i:i+1]).float().to(device)  # (1,6,L)
...
def scenario_predict_local(
    ...
    returns:
      samples: (num_samples, H, D)
    """
    device = torch.device(device if torch.cuda.is_available() else "cpu")
    out_dim = X_last.shape[-1]
    macro_input_dim = macro_x_last.shape[0]  # (6,L)
    ...
    MX_t = torch.tensor(macro_x_last[None]).float().to(device)     # (1,6,L)
...
def rolling_posterior_forecast(
    ...
    with torch.no_grad():
        for t in range(len(X)):
            ...
            preds.append(mean[0, 0, :].cpu().numpy())  # 1-step (첫 horizon)

    return np.array(preds)  # (N, D)


# -------------------------------
# Posterior scenario sampling (around mu_q)
# -------------------------------
def posterior_scenario(
    ...
    with torch.no_grad():
        for _ in range(num_samples):
            ...
            samples.append(mean.squeeze(0).cpu().numpy())  # (H,D)

    return np.stack(samples, axis=0)  # (num_samples, H, D)


# -------------------------------
# Full horizon plotting (history + rolling + scenario fan)
# -------------------------------
...
def compute_coverage_and_sharpness(scenario_samples, true_future, feature_index=0, lower_q=10, upper_q=90):
    ...
    return {f"Coverage_{upper_q-lower_q}%": float(coverage),
