...
if __name__ == "__main__":
    ...
    C_pca = pca_c.fit_transform(C)
    
    print("Explained variance ratio:", pca_c.explained_variance_ratio_)
    
    # ===== Plot PC1 vs PC2 =====
    plt.figure(figsize=(6,5))
    plt.scatter(C_pca[:,0], C_pca[:,1], c=np.arange(len(C_pca)), cmap="viridis")
    plt.colorbar(label="Time index")
    plt.xlabel("PC1")
    plt.ylabel("PC2")
    plt.title("PCA of Condition Vector (C)")
    plt.grid(True)
    plt.show()
    ...
    model.load_state_dict(torch.load("timevae_ctvae_prior.pth", map_location=device))
    model.eval()
    ...
    with torch.no_grad():
        for i in range(len(X)):
            ...
            c_t = torch.tensor(C[i:i+1], dtype=torch.float32).to(device)
    
            mu_q, logvar_q = model.encoder(x_t, c_t)
            Z.append(mu_q.cpu().numpy()[0])
    ...
    # 날짜에서 년도 추출
    years = df_raw.index.year.values
    
    plt.figure(figsize=(8,6))
    
    # PC1 vs PC2 with color by year
    sc = plt.scatter(
        Z_pca[:,0], Z_pca[:,1],
        c=years, cmap="coolwarm", s=40, alpha=0.9
    )
    
    plt.colorbar(sc, label="Year")
    plt.xlabel("PC1")
    plt.ylabel("PC2")
    plt.title("Latent PCA: Business Cycle Structure (Crisis Highlighted)")
    plt.grid(True)
    
    # ====== Crisis periods labeling ======
    crisis_years = {
    ...
    for crisis_year, label in crisis_years.items():
        indices = np.where(years == crisis_year)[0]
        if len(indices) > 0:
            plt.scatter(Z_pca[indices,0], Z_pca[indices,1],
                        c="red", s=80, edgecolors="black", label=f"{crisis_year}: {label}")
